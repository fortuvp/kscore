...
3. Read the complete policy before collecting values. Do not reduce compliance to field validation.
4. Require an explicit ERC-8004 source chain, then verify the numeric agent ID, current agent URI, and current owner at one pinned source-chain block using [references/erc8004-source.md](references/erc8004-source.md). Encode Owner as `eip155:<sourceChainId>:<ownerAddress>`; do not substitute Ethereum mainnet merely because the verification registry is on mainnet.
...
9. Display a real-funds summary. Simulate `ERC20.approve(registry, stake)`, send it after approval, wait for a successful receipt, and re-read allowance.
